...
router.get('/', function(req, res) {
  res.sendFile('index.html', { root: 'public' });//Route to get index.html
});

//Pokemon route
router.get('/pokemon', function(req, res){
	console.log("In Pokemon");
	res.send(pokemon);//res is responce. Will JSON is and everything
});

var politics = "https://zlzlap7j50.execute-api.us-east-1.amazonaws.com/prod";
router.get('/politics', function(req,res) {
  console.log("In politics");
  request(politics).pipe(res);//get any url, pipe it to response and ship to browser
});
